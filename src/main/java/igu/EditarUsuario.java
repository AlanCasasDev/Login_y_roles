...
public class EditarUsuario extends javax.swing.JFrame {
    ...
    private long idUsuarioEditado;

    public EditarUsuario(ControladoraLogica logicControl, Usuario user, long idUsuarioEditado) {
        initComponents();
        this.logicControl = logicControl;
        this.user = user;
        this.idUsuarioEditado = idUsuarioEditado;
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                ...
                                .addGap(124, 124, 124)
                                .addComponent(label_usuario_conectado, javax.swing.GroupLayout.DEFAULT_SIZE, 142, Short.MAX_VALUE))
    ...
    private void btn_guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_guardarActionPerformed
        ...
        if (validarFormulario()) {
            if (!confirmarAccion("¿Desea guardar los cambios al usuario?", "Confirmación")) {
                return;
            }
            ...
            Usuario usuarioEditado = logicControl.traerUsuario(idUsuarioEditado);
            //Set nombre
            usuarioEditado.setUser(user);
            //Set password
            usuarioEditado.setPassword(pass);
            //Ser rol
            List<Rol> listaRoles = logicControl.traerRoles();

            if (listaRoles != null) {
                for (Rol rl : listaRoles) {
                    if (rl.getNombreRol().toLowerCase().equals(rol.toLowerCase())) {
                        usuarioEditado.setRol(rl);
                    }
                }
            }

            if (logicControl.editarUsuario(usuarioEditado)) {
                //metodo de mensaje de usuario creado correctamente
                mostrarMensaje("Se realizo correctamente los cambios al usuario", "info", "Titulo");
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        txt_usuario.setText(usuarioEditado.getUser());
        txt_password.setText(usuarioEditado.getPassword());
        ...
        //Cargar el Rol del usuario selccionado
        for (int i = 0; i < combo_rol.getItemCount(); i++) {
            String rolCombo = combo_rol.getItemAt(i);

            if (rolCombo.equalsIgnoreCase(usuarioEditado.getRol().getNombreRol())) {
